...
export async function installEngine({
 ...
 try {
    ...
    console.error("❌ Failed to clean up installfiles:", err);
  }
...
async function getLocalConfigVersion(
  projectDir: string
): Promise<{ version: string; mono: boolean } | undefined> {

  const gpmrcPath = path.join(projectDir, ".gpmrc");
  const gpmJsonPath = path.join(projectDir, "gpm.json");

  let version: string = "";
  let mono: boolean = false;
