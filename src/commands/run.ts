...
export async function runProject(mode: 'default' | 'test' = 'default') {
	...
	if (!match) {
		console.log(chalk.red(`❌ Engine ${engineVersion} not found in ~/.gpm/engines`));
	...
	try {
		if (mode === 'test') {
			...
			await execa(exePath, ['--headless', '--run-tests', '--path', projectDir], {
 ...
 */
async function findEngineExecutable(engineFolder: string): Promise<string | null> {
	const files = await fs.readdir(engineFolder, { withFileTypes: true });

	const exeFile = files.find((f) => f.isFile() && f.name.toLowerCase().endsWith('.exe'));
	if (exeFile) {
		return path.join(engineFolder, exeFile.name);
	}

	// 2️⃣ Check for macOS .app bundle
	const appFolder = files.find((f) => f.isDirectory() && f.name.endsWith('.app'));
	if (appFolder) {
		const godotPath = path.join(engineFolder, appFolder.name, 'Contents', 'MacOS');
		if (fs.existsSync(godotPath)) {
			const innerFiles = await fs.readdir(godotPath);
			const godotBinary = innerFiles.find((f) => f.toLowerCase().includes('godot'));
			if (godotBinary) {
				return path.join(godotPath, godotBinary);
			}
		}
	}
	...
	const godotBinary = files.find(
		(f) => f.isFile() && f.name.toLowerCase().includes('godot') && !f.name.endsWith('.zip'),
	);
	if (godotBinary) {
		return path.join(engineFolder, godotBinary.name);
	}
