...
export async function initProject() {
	...
	const godotFileParsed = ini.parse(godotFile);
	...
	const answers = await inquirer.prompt([
		{
			...
			default: godotFileParsed.application['config/name'].replace(/"/g, ''),
		},
		{
			type: 'input',
			name: 'description',
			message: 'Description:',
			default: godotFileParsed.application['config/description'].replace(/"/g, ''),
		},
		{
			type: 'input',
			name: 'version',
			message: 'Version:',
			default: godotFileParsed.application['config/version'].replace(/"/g, ''),
		},
		{
			type: 'list',
			name: 'template',
